...
Approvers act only on the single current pending **required** step (`ApproveLeaveRequestStep` / `RejectLeaveRequestStep`). Actionable approvers must have an active employee, linked active user, active company membership, and both `attendance.leave-requests.view` and `attendance.leave-requests.approve`. `attendance.leave-requests.view_all` unlocks privileged manage/visibility for owned-or-scoped mutations (edit/cancel/delete when combined with the matching mutation permission, plus `delete_any` / `reassign_approval`). It does **not** turn Leave Approvals into a company-wide historical list — that history lives in Leave Report.
...
Pending leave requests may be edited only before any **required** approval step has been acted on (Approved / Rejected). Automatic FYI / notify-only `Skipped` rows are terminal for workflow progression but are **not** a human approval action and do not start this edit/delete gate. Pre-action edits are no-op safe, re-check overlap under lock, replace balance reservations with same-key credit only, rebuild the unacted approval snapshot, write a company-scoped audit entry for real edits, and notify the newly resolved current pending approver after commit. After a required approval decision, updates are rejected.

Policy edits can **Sync Pending Requests** (`SyncLeaveApprovalPolicyToPendingRequests`, route `attendance.leave-approval-policies.sync-pending`) from the policy form. Eligible pending leave requests that still resolve to this policy and have **no acted required approval steps** get their unacted approval snapshot rebuilt from the current policy chain. Requests that already started required approvals, completed, or are otherwise ineligible are skipped (preview counts shown in the sync dialog). Sync does not change leave balances, status, or notification settings by itself.

Navigation splits leave into **My leave** (`/attendance/my-leave`, scope fixed to `my`) and **Approvals** (`/attendance/leave-approvals`, scope fixed to `awaiting_my_approval`). `/attendance/leave-requests` redirects into that split. My Leave shows the employee’s own requests plus current-year leave balances when the linked employee is eligible. Approvals is an action queue only — current required pending steps assigned to the signed-in user. The Approvals sidebar badge (`auth.leave_approvals_count`) matches Approvals → **Needs action**. Historical assignment browsing is Leave Report, not Approvals. The badge is company-scoped via trusted `current_company_id` and is `0` unless the user has both `attendance.leave-requests.view` and `attendance.leave-requests.approve`.

Balance operations use focused methods (`reserveIfAvailable`, `releasePendingReservation`, `convertPendingToUsed`, `replacePendingReservation`, `synchronizeBalanceKey`). Approval conversion fails if the pending reservation is missing rather than increasing used alone. `leave-balances:sync` rebuilds `used_days` / `pending_days` from overlapping requests for active leave types and for inactive types that still have balances or historical requests, including multi-year spans (not only `YEAR(start_date)`). It does not overwrite HR entitlement or carry fields.

Annual rollover (`leave-balances:rollover`) runs **daily** at 00:30 in the application timezone and opens each active company's **local business year** via `CompanyTimezone`. A tenant is processed only when its local date is already in that year; New York does not open the next year while still on Dec 31 when UTC has crossed into January. Rollover remains idempotent via `leave_balances.rollover_applied_at`. Existing balances for year ≤ company business year are treated as already-opened (corrective backfill); future provisional years stay null until rollover applies carry once without wiping pending/used. Explicit `php artisan leave-balances:rollover {year}` still forces that year.

Leave type presentation (`LeaveTypeYearBalance`) exposes `base_entitlement_days`, `carried_days`, `total_available_days`, plus a compatibility `entitled_days` alias equal to the total pool (base + carry). Database `entitled_days` remains base entitlement only. **Historical calendar/years** (requested year before the company's current business year via `CompanyTimezone`) are read-only: they never call `ensureEmployeeYear`, never invent entitlement from today's `LeaveType.days_per_year`, and present only persisted `LeaveBalance` rows (including inactive/soft-deleted leave-type metadata when safely available).

Department.include_in_attendance_leave controls whether employees currently assigned to that department participate in Attendance and Leave. It is direct department configuration only (no parent inheritance). Existing departments remain included after migration; newly created departments default to excluded until an administrator enables them (DB column default false; application create also defaults false when omitted). Attendance/Leave queries combine this gate with EmployeeVisibilityScope for normal manager and report access. Leave Approvals always require the department gate; view_all actors are also limited by EmployeeVisibilityScope, while ordinary assigned approvers may still act on enabled-department requests outside their directory visibility. Disabling participation, deleting a department, or moving an employee to an excluded/null department is blocked while that department’s / employee’s LeaveRequests remain pending (`DepartmentAttendanceLeaveGuard`). Participation transitions and leave submission serialize on department/employee row locks inside DB transactions so concurrent submit vs exclude/move cannot strand a pending Leave request. Successful participation/move mutations invalidate dashboard caches (including user-scoped personal/leave/attention keys) via `DashboardAnalytics::forgetCompany`. The personal dashboard also respects this gate and does not provision Leave balances merely by viewing the dashboard for an excluded employee. This setting does not delete data, change Crew Operations, general employee visibility, or Hikvision ingestion.
...
Leave types also store a stable reporting `category` (`annual`, `sick`, `other`). Leave Report and Leave Balance Report use that category for filtering and export (and may still compute category day buckets in the Leave Report payload). It is independent of `payroll_treatment` and of the editable name or code. Existing rows default to `other` until HR classifies them. The category does not change how `days_per_year` becomes `LeaveBalance.entitled_days`. Leave Report’s on-screen cards filter by leave type with request counts; they are not Annual/Sick day-total summary cards.
...
`leave-balances:sync` repairs historically relevant keys (existing balances + approved/pending requests), including inactive/terminated employees with history. It does **not** invent entitlement for missing historical years from today's `LeaveType.days_per_year`; those anomalies are reported (exception log + command WARNING with identifying rows) and skipped while the command still exits SUCCESS so deployments are not blocked by unavailable historical provenance. Normal current/future provisioning for active employees and active leave types continues via `ensureEmployeeYear`.

Privileged administrators with `attendance.leave-requests.view` + `view_all` + `reassign_approval` may **reassign the current required Pending approval step** via `ReassignLeaveRequestApproval` when the current approver can no longer act (inactive employee/user, lost membership/permissions, incorrect selection, or otherwise stuck). The action locks the request and approvals, reassigns only that Pending required row’s `approver_employee_id` / `approver_user_id`, leaves sequence/status/`acted_at`/policy provenance untouched, does not rebuild Waiting steps or the policy snapshot, and does not create or mutate leave balances (read-only integrity inspection only). Before reassignment it verifies each affected balance key’s `pending_days` matches the company-scoped aggregate pending allocation from real pending requests (within the balance manager numeric tolerance); missing or inconsistent pending ledgers are rejected with a user-safe validation message and are not auto-repaired. Stale dialogs are rejected via `expected_approval_id` (+ expected current approver). Duplicate approvers are forbidden across **all** snapshot steps including FYI. Append-only `leave_request_approval_reassignments` rows keep name snapshots and survive approval-row deletion (`nullOnDelete` on the approval FK after the corrective nullable migration; production originally shipped NOT NULL + cascade and is upgraded additively). One company-scoped LeaveRequest activity event records the user-facing audit. After commit, the new approver receives the existing action-required email when company notification settings and the template allow it.
...
| Attendance / leave | `/attendance/*`, `/organization/reports/leave`, `/organization/reports/leave-balances` | `records`, `calendar`, `types`, `my-leave`, `leave-approvals`, `leave-approval-policies`, `leave-approval-settings`; reports under `organization/reports/leave*` |
