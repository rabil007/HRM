...
Annual rollover (`leave-balances:rollover`) is idempotent via `leave_balances.rollover_applied_at`. Future-year provisional balances may exist before January 1 (from early reservations); when rollover runs it applies carry-forward once without wiping pending/used. Re-running rollover for the same year does not duplicate carry.
