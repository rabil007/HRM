...
Leave type presentation (`LeaveTypeYearBalance`) exposes `base_entitlement_days`, `carried_days`, `total_available_days`, plus a compatibility `entitled_days` alias equal to the total pool (base + carry). Database `entitled_days` remains base entitlement only.

Leave types store a stable `payroll_treatment` (`paid` / `unpaid`) used by Office Payroll. Renaming code or display name does not change salary calculation. Legacy codes `UL`, `UNPAID`, and `LOP` were backfilled to unpaid on migration.
