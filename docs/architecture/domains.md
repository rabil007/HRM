...
- New operational Crew activity (assignments, Join/Transfer/Redeploy, Planning) cannot use legacy-unassigned Vessels (`vessel.client_id = null`) or inactive Vessels. When a draft/assignment includes a Vessel, `CrewAssignment.client_id` is snapshotted from that Vessel’s current Client after company-ownership and active-status checks.
- Editable pre-P4 Crew Assignments may keep an unchanged legacy Vessel value during unrelated edits; selecting a new Vessel/Client must satisfy today’s operational rules.
