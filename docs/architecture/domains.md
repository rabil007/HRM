...
Department.include_in_attendance_leave controls whether employees currently assigned to that department participate in Attendance and Leave. It is direct department configuration only (no parent inheritance). Existing departments remain included after migration; newly created departments default to excluded until an administrator enables them. Attendance/Leave queries combine this gate with EmployeeVisibilityScope for normal manager and report access. Leave Approvals always require the department gate; view_all actors are also limited by EmployeeVisibilityScope, while ordinary assigned approvers may still act on enabled-department requests outside their directory visibility. This setting does not delete data, change Crew Operations, general employee visibility, or Hikvision ingestion.
