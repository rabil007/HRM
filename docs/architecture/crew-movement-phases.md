...
- If Home is supplied, all movement periods must be closed → Completed assignment.
- If Home is not supplied, exactly the latest chronological period may remain open (empty To) → Active assignment.
- All periods closed with no Home is rejected as ambiguous current state.

Partial history is acceptable (for example open Onsite alone creates Active P4 without fabricating earlier standby/training).
...
- Open/current bootstrap still requires an Active HR employee; completed history remains allowed for inactive/terminated employees.

### Add Past Crew Data Dialog & Manual Entry Flow

On `/organization/crew`, users with the authoritative permission `crew_operations.assignments.create_historical` see an **Add Past Data** action beside **Start Assignment**. Form options for historical employees are isolated in `historical_form_options` and only delivered when authorized. User-facing permission label may read **Record Past Crew Data**; the permission key is unchanged.
...
### Payroll and Sea Service

Past Data changes Crew Assignment operational history only. It does **not** write `PayrollPeriod`, `CrewTimesheet`, `PayrollRecord`, or `PayrollWorkAllocation` directly.

Later Crew payroll Draft → Populate / Refresh reads `CrewAssignmentPhase` dates:
...
Existing `CrewAssignment` / `CrewAssignmentPhase` history is untouched and remains readable. Past Data does not migrate old detailed event history into a new format. The new simplified period representation is the authoritative UI/import path.
