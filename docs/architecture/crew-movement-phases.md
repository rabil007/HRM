...
Accommodation master data is managed under Settings → **Hotels**. Each Hotel owns its **Room Types** (`Hotel` → `RoomType`). A Room Type is hotel-specific, not company-global; room type names are unique per hotel (`company_id + hotel_id + name`). Hotels remain company-scoped. Nested Room Type management (create, edit, deactivate, remove unused) requires `settings.master-data.hotels.update`; whole-Hotel deletion remains `settings.master-data.hotels.delete`. Hotel updates never infer Room Type deletion from a missing `room_types` payload — removals require explicit `removed_room_type_ids`. Unused Hotels and unused Room Types are hard-deleted; referenced master data cannot be deleted and should be deactivated with `is_active` instead. Hotel deletion re-checks usage inside the same locked transaction (the pre-delete UX check is not authoritative). Legacy assignment and multi-hotel split reconciliation are transactional and row-locked; when a destination Hotel already has a same-name Room Type, reconciliation reuses it instead of creating a duplicate. Historical `CrewAccommodationStay` updates during reconciliation save through Eloquent so integrity checks and activity history remain auditable. Legacy `room_types` rows may still exist with a null `hotel_id` until reconciled from the Hotels index (automatic single-hotel assignment or split-by-hotel for multi-hotel history); ambiguous or conflicting legacy data stays unresolved for manual reconciliation rather than being guessed. New room types and new accommodation selections must belong to the selected Hotel. Historical stays that still reference a transitional null-`hotel_id` Room Type remain readable until reconciled. A single assignment may have multiple accommodation stays, including multiple records of the same `stay_type` (for example hotel changes within one mobilisation cycle). There is no `unique(crew_assignment_id, stay_type)` constraint.
...
|------|-------|---------------------|
| P0 | Pre-Mobilisation | Assignment prepared and waiting for the crew member to arrive. |
...
- **P0** is pre-mobilisation waiting for arrival, not boarding.
...
Start Crew Assignment (/organization/crew/create)
    ...
    ↓ per-employee Rank and Arrival Date
...
| Per-row fields | Employee, Rank, and Arrival Date (`planned_arrival_at`). Rank still defaults from the employee profile. |
...
3. **Timestamp Interpretation & Double-Conversion Protection**:
   - Timestamps with explicit timezones (e.g. `2026-09-17T19:30:00Z`, `+04:00`, `+00:00`) represent absolute instants and are converted to company timezone.
   - Timestamps without timezone offsets (e.g. `2026-09-17 23:30:00`, `2026-09-17T23:30`) represent company-local wall-clock values directly and must not be double-converted or have `Z` appended.
   - `CrewAssignmentPresenter` resolves timezone via canonical `CompanyTimezone::forCompany($assignment->company ?? (int) $assignment->company_id)`, guaranteeing presenter timezone = movement context timezone = form request timezone = future-actual guard timezone.
...
## Historical Crew Data Management (Add Past Data)

High-trust maritime operators need to **reconstruct old crew movement history** and, when safe, **bootstrap where the employee currently is** in OMS-HRM. Sea Service is a downstream result of a completed On Vessel → Disembarked (P4) period — not the primary reason the feature exists.

### Distinction: Live vs Historical Workflows
...
- **Historical Data (`Add Past Data`)**: Enter known historical **movement events**. OMS reconstructs phase intervals with the **same transition semantics as Crew Operations** (`HistoricalPhaseBuilder` → `HistoricalCrewAssignmentService`). Does **not** replay live side effects for past transitions (no retroactive alerts, hotels, planning, notifications). When the reconstructed journey ends in an open phase and no conflicting Active OMS assignment exists, the imported assignment may become **Active** so normal Crew Operations can continue from that point.
...
- **Manual Entry**
- **Import Excel** (template download, validation preview, confirmed bulk import, batch history)
...
Domain validator keys such as `training_start_at`, `training_end_at`, and `mobilisation_start_at` are aliased to Manual Entry form fields (`training_started_at`, `training_ended_at`, `mobilisation_at`). Domain failures (`overlap`, `sea_service`, `assignment`, `dates`, `workbook`) are shown in the visible validation alert with the full backend message — never reduced to a generic “Validation failed.”

#### 2-Step Authoritative Flow: Enter → Validate → Preview → Confirm → Persist

1. **Enter**: Operator enters:
   - **Assignment**: Employee `*`, Vessel `*`, Rank `*`, Client (optional historical snapshot)
   - **Before Vessel**: Pre-Mobilisation, Join Standby, Training Start, Training End
   - **Vessel**: On Vessel, Disembarked (optional — not universally required)
   - **After Vessel**: Home / Redeployment
   - **Notes**: Remarks
   - At least one meaningful movement date is required. Unknown dates must not be guessed.
   - **Removed inputs** (not shown; Manual `prohibited`; Excel outdated-template rejection): Travel In, Ready to Join, Post-Training Join Standby, Demobilisation Standby, Assignment Closed.
   - **Travel In (P1)** and **Ready to Join (P3)** remain readable only for old database compatibility.
2. **Validate**: Client calls `POST /organization/crew/historical/preview`. The backend authoritatively runs domain validations and `HistoricalPhaseBuilder`. No database records are written.
3. **Preview**: Dialog renders:
   - Prominent **Inferred Current State** from the backend (not computed independently in the browser)
   - Summary of employee, vessel, rank, client, remarks, assignment Active vs Completed
   - Reconstructed timeline with open phases ending as **Current**
   - Sea Service impact (`Will Create`, `Will Link`, `Conflict`, `Disabled`, or `not_applicable` when P4 is still open)
   - Verification checklist and non-blocking warnings
4. **Confirm & Persist**: `POST /organization/crew/historical` locks the employee, revalidates, writes assignment + phases, syncs Sea Service only for completed P4, and audits with `activity_log.company_id`.

### Event → phase mapping (canonical)

Manual Preview/Store and Excel Preview/Import share `HistoricalPhaseBuilder`:

| Last valid event | Inferred state | Assignment |
| --- | --- | --- |
| Pre-Mobilisation | P0 Pre-Mobilisation (open) | Active |
| Join Standby | P2A Join Standby (open) | Active |
| Training Start | P2B Training (open) | Active |
| Training End | P2A Join Standby (open) — same meaning as live `complete_training` | Active |
| On Vessel | P4 On Vessel (open) | Active; **no** Sea Service yet |
| Disembarked | P5 Demobilisation Standby (open) — Disembarked **is** the P5 start | Active; Sea Service for completed P4 |
| Home / Redeployment | P6 / completion — same semantics as live `travel_home` (Close) | Completed; `closed_at` set |

Derived boundaries are allowed only where the live movement action itself defines both phase end and next phase start (Training End → P2A, Disembarked → P5, Home → P6). Do not fabricate Pre-Mobilisation, Training Start, On Vessel, or Home when those events were never supplied.

**Direct Disembark → Home** (same timestamp): reconstruct as live P4 → P6 — do not manufacture a positive-duration P5.

Chronology is validated in **canonical journey order** before inference. Invalid order (e.g. Training Start after Join Standby reversed) is rejected — never inferred via `max(timestamp)`.

### Existing Active OMS assignment protection

If the employee already has an Active Crew Assignment, an imported/manual row that would become another Active/open assignment is blocked:

> This employee already has an active Crew Assignment in OMS-HRM. Historical records before the active assignment may still be imported, but this row cannot become another current assignment.

Completed non-overlapping history may still be imported. Current OMS Active assignment remains authoritative.

### Excel multi-row rules

Per employee, rows are sorted chronologically. Validate: no overlapping intervals; at most one open assignment; open assignment must be the chronologically latest; no later row after an open row.
...
1. **Company Timezone Semantics & Sea Service Calendar Dates**:
   ...
   - Future actual timestamps are strictly rejected.
   - Sea Service calendar dates use company-local dates without early UTC drift.
2. **Dependencies (current-flow parity)**:
   - Training End requires Training Start; Training End cannot be before Training Start.
   - Disembarked requires On Vessel; Disembarked must be after On Vessel (existing P4 duration rule).
   - Home / Redeployment (post-vessel) requires Disembarked and must not precede it (equal timestamp = direct P4 → P6).
3. **Zero Invented History**: Only supplied events are persisted; optional Training may be omitted entirely (e.g. P0 → P2A → P4).
4. **Overlap**: Completed intervals use half-open overlap helpers. Open/bootstrap intervals extend to current and must not overlap another Active assignment, a later assignment, or another open imported row.

### Operational / bootstrap side effects

- Do **not** retroactively manufacture old alerts, hotel stays, planning rows, or notifications for reconstructed past transitions.
- An imported **Active** historical assignment must remain compatible with normal `CrewMovementService` actions from the inferred current phase (e.g. Active P2A → Send to Training / Join Vessel; Active P4 → Confirm Disembarkation; Active P5 → Travel Home).
- Payroll/timesheet continues to use actual phase intervals and existing payable-phase classification — historical import must not make Pre-Mobilisation, Home, or legacy phases payable merely because they were imported.

### Sea Service Synchronization & Deduplication

- Sea Service is created/linked **only** for a completed On Vessel → Disembarked period (completed P4).
- Open Active P4 does **not** invent an end date or Sea Service record.
- **Exact Match Resolution** (`HistoricalSeaServiceMatchResolver`) and inclusive calendar overlap protection are unchanged.
- Historical Rank and Client snapshots are preserved on assignment and Sea Service linking.
...
Fill Excel (Historical Assignments + Reference Data)
       ↓
Upload → parse → resolve identities
       ↓
HistoricalCrewAssignmentData → HistoricalPhaseBuilder / Validator
       ↓
Workbook duplicate / overlap / open-assignment rules
       ↓
Validation Preview (Ready / Warning / Blocked + Last Movement + Inferred State)
```

#### Template workbook

Assignment columns: `Employee No *`, `Employee`, `Vessel *`, `Rank *`, `Client`, `Pre-Mobilisation`, `Join Standby`, `Training Start`, `Training End`, `On Vessel`, `Disembarked`, `Home / Redeployment`, `Remarks`.

Outdated templates containing Travel In, Ready to Join, Post-Training Join Standby, Demobilisation Standby, or Assignment Closed (or their internal keys) are rejected with a template-version message.

Instructions explain event-based reconstruction, Training End → Join Standby, Disembarked → Demobilisation Standby, Sea Service only after completed vessel service, and that unknown dates must not be guessed.

Reference Data keeps Employees / Vessels / Ranks / Clients with visibility/tenancy protections. Vessel → **Current Client** remains clearly labelled because historical Client may differ.
...
| Warning | Inactive vessel/rank/client, terminated employee, Sea Service sync disabled, historical Client ≠ Vessel current Client |
| Blocked | Missing identity, chronology errors, Active OMS conflict for open bootstrap, Sea Service conflict, workbook duplicate / overlap / multiple open rows, outdated template, formula cells |

Workbook limit remains **5,000** non-empty rows. Sample `EXAMPLE001` rows are ignored.
...
Hardened behavior is preserved: server-side reparse, authoritative revalidation, idempotency, workbook hash, resume/stale batch handling, atomic row result + assignment persistence, result workbook, batch history.

Result workbook columns: Row, Employee No, Employee, Vessel, Rank, Last Movement, Inferred State, Import Status, Warnings, Errors, Assignment No.
...
| `crew_assignments.historical_import_batch_id` | Traceability |
| `source` | `historical_import` / `historical_manual` |

Do **not** migrate/delete previously stored phases (including legacy P1/P3) merely because the new input format is simpler.
...
- `GET /organization/crew/historical/import/batches/{batch}/result`
...
Status, `current_phase_id`, and open vs completed semantics are **server-derived**. Never trust client-submitted `company_id`, `current_phase`, or `status`.
