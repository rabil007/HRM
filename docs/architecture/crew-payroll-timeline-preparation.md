...
Approval does not grant permission to apply stale data: if a preparation becomes stale between approval and apply, application blocks and requires re-preparation and re-approval. `ApplyCrewTimesheetPreparation` locks participating employees, crew assignments, period-relevant source phases, pending corrections on those phases only, and crew contracts in deterministic order (`Employee → Assignment → Phase → Correction → Contract`), loads the locked current source into `LockedCrewTimelineSource`, and re-validates freshness with `hashLockedSource()` so the final Apply hash fingerprints the same logical source scope as the ordinary freshness hash without ordinary re-queries under the transaction snapshot.

**Apply employee lock boundary:** employees whose Crew source could affect the preparation or payroll period, merged from (1) current `issuePhases`, (2) existing preparation lines, and (3) employees with a crew contract resolvable for the period via `ResolveCrewContractForPayrollPeriod::crewEmployeeIdsResolvableForPeriod()` (same overlap and legacy fallback rules as contract resolution). Employees with no resolvable crew contract are intentionally excluded: they cannot produce payable crew timesheet lines until a crew contract exists, and their first crew assignment is serialized through the employee lock whenever another protected employee’s Apply transaction is active; an empty approved preparation applied while no resolvable-contract employees remain in the boundary cannot be affected by that edge case in payroll outcomes.
