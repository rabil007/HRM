...
**Apply employee lock boundary:** `CrewPayrollSourceEmployeeBoundary::stableEmployeeIds()` defines the stable active-employee population serialized before source discovery. It merges (1) preparation line employees, (2) employees with a crew contract resolvable for the period, (3) active employees in the current crew payroll universe (`PayrollEmployeeQuery::activeQuery(Crew)`), (4) active employees with any crew assignment history, and (5) active employees with any crew contract history. When that merged set is empty — for example the first crew source in a company with no prior crew contracts or assignments — Apply falls back to locking all active company employees so the first concurrent Crew Assignment or Crew Contract creation cannot race unnoticed through final Apply. The lock boundary may be broader than the hash payload; locks protect potential concurrent source creation while the hash contains only actual period-relevant logical source.
