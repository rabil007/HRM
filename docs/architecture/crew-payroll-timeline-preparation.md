...
CrewTimesheetPreparation
    ↓ versioned review / warning / approval workflow
...
- **`CrewAssignment` / `CrewAssignmentPhase`** remains the operational source of truth for movements.
- **Crew Timesheet preparation** is a Payroll workflow that extracts actual movement dates into versioned, reviewable timesheet lines. It is not a replacement for Crew Assignment.
- Planned schedule dates are never payroll movement actuals.
- `source = crew_operations` (`CrewTimesheetSource::CrewOperations`) remains an internal persisted compatibility database value.
- Technical `CrewTimeline*` naming (`app/Support/Payroll/CrewTimeline/`, `CrewTimelinePhaseQuery`, `payroll.crew-timeline.*`) remains for implementation compatibility; user- and product-facing copy refers to **Crew Timesheet** and **Crew Assignments**.
...
- The Payroll Crew Timesheet review payload must not expose planned schedule dates as phase fields. Payroll review presents only actual movement dates, payroll allocation ranges, and warning affected periods.
...
Persistent, additive audit table for employee Crew Timesheet skips:
...
Approval does not grant permission to apply stale data: if a preparation becomes stale between approval and apply, application blocks and requires re-preparation and re-approval. `ApplyCrewTimesheetPreparation` locks a stable crew employee population first, then crew assignments, period-relevant source phases, pending corrections on those phases only, and crew contracts in deterministic order (`Employee → Assignment → Phase → Correction → Contract`), re-discovers period-relevant source under those locks into `LockedCrewTimelineSource`, and re-validates freshness with `hashLockedSource()` so the final Apply hash fingerprints the same logical source scope as the ordinary freshness hash without ordinary re-queries under the transaction snapshot.

**Apply employee lock boundary:** `CrewPayrollSourceEmployeeBoundary` separates two populations that are unioned into `stableEmployeeIds()` before `Employee FOR UPDATE`:

1. **`existingSourceEmployeeIds()`** — employees already represented by the current preparation or historical Crew source (preparation lines, resolvable crew contracts for the period, crew assignment history, historical crew contracts). These remain in the Apply integrity boundary even when the employee is inactive or terminated.
2. **`potentialNewSourceEmployeeIds()`** — every active company employee, because any active employee can start a first Crew Assignment or receive a first Crew payroll contract during Apply.

The lock boundary is intentionally broader than the hash payload: locks protect potential concurrent source creation while the hash contains only actual period-relevant logical source (phases, resolvable contracts, pending corrections on period-relevant phases).

**Locked Apply hash:** `CrewTimelineSourceLocker` builds `LockedCrewTimelineSource` with period-relevant phases, resolved contracts from locked crew contract rows, `contractFingerprintEmployeeIds` derived from that locked collection, and pending corrections on those phases. `CrewTimelineSourceHasher::hashLockedSource()` fingerprints only that supplied payload and performs no additional ordinary source queries.

**Crew Movement transaction entry:** `CrewMovementService::perform()` resolves tenant-scoped assignment identity before opening the transaction, then locks `Employee → Assignment` and reloads the current assignment graph under those locks.

**Crew Contract creation:** `UpsertEmployeeContract` acquires an `Employee` row lock before creating or materially updating a Crew payroll contract (`Employee → Contract`).
...
The Payroll Crew Timesheet review payload must not expose planned schedule dates as phase fields. Payroll review presents only actual movement dates, payroll allocation ranges, and warning affected periods. Planned dates are intentionally not shown in the payroll breakdown because they are not payroll inputs.
...
## Phase 1D apply approved Crew Timesheet to timesheets
...
- Direct editing of generated preparation lines
