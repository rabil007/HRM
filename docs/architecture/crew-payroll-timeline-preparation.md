...
Approval does not grant permission to apply stale data: if a preparation becomes stale between approval and apply, application blocks and requires re-preparation and re-approval. `ApplyCrewTimesheetPreparation` locks participating employees, crew assignments, phases, pending corrections, and crew contracts in deterministic order, loads the locked current source into `LockedCrewTimelineSource`, and re-validates freshness with `hashLockedSource()` so the final Apply hash uses locked phases, contracts, and pending corrections without ordinary re-queries under the transaction snapshot. When no issue phases exist yet, the lock boundary still includes employees with period-overlapping crew contracts so new relevant crew source cannot appear outside Apply.
