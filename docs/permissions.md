...
Unified document signing and acknowledgement (Phase 6A) uses `documents.recipient-requests.view|create|cancel`. Phase 6B-1 / 6B-2A / 6B-2B2 add `documents.recipient-requests.respond` for authenticated company users assigned as **department manager** or **company signatory** on a specific request — including repeated advanced-flow stages (Manager 2, Director/CEO as labeled company signatories). Respond does not grant company-wide recipient request browsing. These permissions are separate from legacy `bulk_documents.signatures.review` and from Phase 5 workflow permissions. Token regeneration reuses `documents.recipient-requests.create` and applies only to public subject-employee requests. Public `/document-action/{token}` completion requires no authenticated permission; company scope is derived from the persisted request bound to the token hash (including Phase 7A delivery-specific subject email tokens). Manager and company countersigning use authenticated internal routes only.
