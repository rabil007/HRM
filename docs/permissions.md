...
| Reports                                                  | `reports.crew_movement_history.view\|export`, `reports.leave.view\|export`, `reports.leave_balance.view\|export\|update_opening`                                                                                                                                                                                                                                                                                                                                              |
| Attendance / leave                                       | `attendance.overview.view`, `attendance.records.*`, `attendance.types.*`, `attendance.leave-requests.*` (incl. `view_all`, privileged `delete_any`, and privileged `reassign_approval`; approve is step-scoped; Leave Approvals shows only the current required pending step), `attendance.leave-approval-policies.*`, `attendance.leave-approval-settings.view\|update` (company-scoped approver defaults and leave-request email notification switches; Email Templates still control content/enabled state) |
...
| Recruitment                                              | `recruitment.requirements.view|create|update|close|cancel|reopen|attachments.download`                                                                                                                                                                                                                                                                                                                                                     |
| Audit                                                    | `audit.view`                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `attendance.leave-requests.reassign_approval` | Privileged exceptional recovery: reassign **only** the current required `Pending` approval step to another eligible approver. Requires `view` + `view_all` + `reassign_approval`. Does **not** grant general employee directory visibility, permission to approve on behalf of the new approver, ability to modify completed approval decisions, or ability to rebuild an approval policy snapshot. Preserves prior Approved history, Waiting future steps, policy provenance, and leave balances (read-only integrity; never creates balances). Stale dialogs must send matching `expected_approval_id`. Append-only history in `leave_request_approval_reassignments` survives approval-row rebuild (`nullOnDelete`); activity log records one user-facing event. |

Ordinary `delete` must never be broadened to cover approved or mid-approval requests. Administrative deletion uses route `attendance.leave-requests.administrative-destroy` and row capability `can_administratively_delete`. Approval reassignment uses route `attendance.leave-requests.reassign-approval` and row capability `can_reassign_current_approval`.
...
Signing preset management uses `documents.signing-presets.*`. Starting a signing flow from an active signing preset requires `documents.recipient-requests.create` (not signing-preset CRUD). Retry uses create; cancel uses `documents.recipient-requests.cancel`. Explicit Send / Resend of recipient-request email (Phase 7A) also uses `documents.recipient-requests.create` — no separate email permission. Company reminder automation settings (Phase 7B) use `documents.recipient-automation.view|update` and do not grant signing, resend, or request creation.

Unified document signing and acknowledgement uses `documents.recipient-requests.view|create|cancel`. `documents.recipient-requests.respond` is for authenticated company users assigned as **department manager** or **company signatory** on a specific request — including repeated advanced-flow stages. Respond does not grant company-wide recipient request browsing. These permissions are separate from Phase 5 workflow permissions and from Salary Certificate bulk generate/email (`bulk_documents.*`). Token regeneration reuses `documents.recipient-requests.create` and applies only to public subject-employee requests. Public `/document-action/{token}` completion requires no authenticated permission; company scope is derived from the persisted request bound to the token hash (including Phase 7A delivery-specific subject email tokens). Manager and company countersigning use authenticated internal routes only.
...
Automatic Spatie activity logging now covers a broad set of organization, master-data, employee sub-record, crew, attendance/leave, and payroll models through `LogsActivityWithCompany`. The implementation is the source of truth: search `app/Models` for the trait rather than maintaining a fragile model count here. Operational events that are not model CRUD may be logged manually by services. Company Document expiry recipient list changes are one explicit event (`company_document_expiry_notification_recipients_updated`) rather than a row per pivot insert/delete.
