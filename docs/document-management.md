...
Legacy Salary Declaration BulkDocumentSignatureRequest rows and files are retained for audit/history but have no user-facing runtime. There is no row-to-row migration into `DocumentRecipientRequest`. Statuses, tokens, signed PDFs, and review fields are never rewritten by the read-only cutover command.
...
1. Confirm any remaining production legacy `submitted` review is finished or explicitly accepted as frozen history.
2. Deploy this cleanup (no `/esign/*`, no Settings E-Signature, no Requests legacy tab, no admin bulk signature review UI).
3. Run the read-only legacy report if operators still need an export of historical awaiting employees.
...
Do not delete historical tables or files. The read-only `documents:legacy-signatures-cutover` command remains an audit/export tool.
...
1. **Company Templates** (`document_generation_templates`):
   ...
   - **Unified PDF Designer** (`/{template}/design`):
     ...
     - **PDF Overlay layout preflight**: drafts may be saved with layout issues, but publishing runs an authoritative sample-data layout preflight using the same Chromium measurement engine as document generation (`PdfOverlayLayoutPreflight`). Results have a status of `valid`, `invalid`, or `unavailable`. Real overflow (`LAYOUT_OVERFLOW`) is shown against the physical placement in the Unified Designer (page, field label, and placement id). Source PDF problems stay `TEMPLATE_SOURCE_UNAVAILABLE`. Invalid `placement_config` stays `TEMPLATE_LAYOUT_CONFIGURATION_INVALID`. If Chromium/Browsershot cannot complete measurement, the result is `unavailable` (`TEMPLATE_LAYOUT_VALIDATION_UNAVAILABLE`) with a diagnostic reference (`LAY-…`) — this is not counted as a layout issue and no field is highlighted. Optional **Preview with employee** uses `DocumentTemplateMergeFields::valuesForEmployee()` for one selected company employee (`employees.view` required) and does not write the design. Runtime generation still validates again. Historical published and archived versions are not rewritten.
     ...
     - **Publish**: Server-gated by readiness plus an authoritative PDF Overlay sample-data layout preflight, plus existing lifecycle/signature validation. Unsaved Designer changes must be saved first; Save Draft and Publish stay explicit. Save Draft is allowed when layout preflight is invalid or unavailable. Publish is blocked for both real layout overflow and validation-engine unavailability; the HTTP codes differ (`TEMPLATE_LAYOUT_INVALID` vs `TEMPLATE_LAYOUT_VALIDATION_UNAVAILABLE`).
...
| `/organization/documents/templates/{template}/versions/{version}/validate-design` (POST) | Read-only layout preflight for the current canvas or persisted draft (sample or one employee) | `documents.templates.update` (employee mode also needs `employees.view`) |
...
Every non-empty placement is measured in Chromium after `document.fonts.ready`. Overflow is detected when `scrollWidth > clientWidth + 1` or `scrollHeight > clientHeight + 1`. If the requested size does not fit, the renderer shrinks by `0.25pt` down to `8pt` so the value stays inside the drawn box. If the value still overflows at `8pt`, generation is blocked with `DocumentTemplateLayoutException`. Values are not clipped, truncated, or ellipsized. Empty merge values render nothing. The designer treats the drawn box as the max size. Authoritative pre-publish validation uses sample merge values (and optionally one employee) with the same Chromium measurement as generation. **Layout invalid** means the template configuration or content does not fit. **Validation unavailable** means the authoritative layout engine could not complete the check (Chromium/Puppeteer/Node/Browsershot failure, malformed measurement response). Both block publish. Unavailable failures are logged as `document_template_layout_preflight_failed` with `company_id`, `template_id`, `version_id`, exception class/cause, binary-availability booleans, and a `LAY-…` reference returned to the Designer — not employee merge values, PDF contents, or filesystem paths. Runtime generation validation remains active. Values are not clipped, truncated, or ellipsized. Empty merge values render nothing.

Preflight runs for all placements before any overlay PDF or canonical/Library file is written. Designer validation reports every overflowing physical placement in one pass. Layout failure during generation logs `company_id`, `generation_run_id`, `run_item_id`, `employee_id`, `template_id`, `template_version_id`, `error_code`, `placement_id`, `field_key`, and `page` — not filesystem paths or employee PII. The run item stores a field-specific message such as "Employee Full Name does not fit the configured field on page 1." Static text uses "A text box does not fit the configured area on page N."
...
- **Signing** — `DocumentRecipientRequest` inbox (`tab=recipient`). Includes subject employees and manager / company signatory requests. Rows show: employee, document, waiting for (recipient name + role), human status, requested timestamp.

There is no Legacy Signature Requests tab. Historical `BulkDocumentSignatureRequest` rows are not listed.

**Human-Readable Status**: Presenters normalize backend states into user-friendly sentences. Approvals use Waiting for Review/Approval plus Approved, Rejected, Cancelled. Signing uses Waiting for Sign/Acknowledge plus Email delivery failed, Expired, Cancelled, Superseded, Completed. Recipient requests have no `rejected` status.
...
Legacy `/organization/documents/bulk?view=signatures` redirects into current Requests Signing when authorized; it never renders the historical roster.
...
No manager/countersigning, external recipients, email/reminders/WhatsApp/push, automatic workflow sign stages, candidate signing, or automatic template→preset routing were in the original Phase 6A slice. Current Salary Declaration signing uses Company Templates and this unified recipient path.
