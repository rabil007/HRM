...
1. **Company Templates** (`document_generation_templates`):
   ...
   - **Unified PDF Designer** (`/{template}/design`):
     ...
     - **Publish**: Server-gated by readiness plus an authoritative PDF Overlay sample-data layout preflight, plus existing lifecycle/signature validation. Unsaved Designer changes must be saved first; Save Draft and Publish stay explicit. Save Draft is allowed when layout preflight fails.
