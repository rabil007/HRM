...
1. **Company Templates** (`document_generation_templates`):
   ...
   - **Unified PDF Designer** (`/{template}/design`):
     ...
     - **Vertical alignment**: Each field/text placement stores `vertical_align` (`top` / `middle` / `baseline`). New boxes default to `baseline`. Existing placements without the key keep the previous look (`middle` for merge fields, `top` for static text). Generated overlays map those to `align-items: flex-start|center|flex-end`.
   ...
   - **Employee Signature Placement**:
     ...
     - Drag/resize persists the geometric box (`left`/`top`/`width`/`height` with scale baked in), not Fabric `getBoundingRect()`, so the outline stroke does not shift saved coordinates.
