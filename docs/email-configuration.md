...
The same configuration applies to all organizational documents (both Company Documents and Branch Documents) for that company that have an expiry date. There are no per-document or per-branch recipient overrides; branch documents inherit the company-level recipients.

### Scope and Aggregation

Only organizational documents that satisfy all of the following are eligible:

1. Belong to the active company (either company-level `branch_id IS NULL` or branch-level `branch_id IS NOT NULL`)
...
Alert emails aggregate both company-level and branch-level documents, explicitly labeling each document's scope in a dedicated **Scope** column (`Company` or `{Branch Name}`) and providing links to the corresponding document view. On the Branch Documents page, operators are informed that expiry notification recipients are configured at the company level.
