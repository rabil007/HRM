...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (! Schema::hasIndex('crew_timesheet_preparations', 'ctp_company_period_version_uq')) {
            Schema::table('crew_timesheet_preparations', function (Blueprint $table) {
                $table->unique(
                    ['company_id', 'payroll_period_id', 'version'],
                    'ctp_company_period_version_uq',
                );
        ...
        if (! Schema::hasIndex('crew_timesheet_preparations', 'ctp_company_status_idx')) {
            Schema::table('crew_timesheet_preparations', function (Blueprint $table) {
                $table->index(['company_id', 'status'], 'ctp_company_status_idx');
            });
        }

        if (! Schema::hasIndex('crew_timesheet_preparations', 'ctp_period_status_idx')) {
            Schema::table('crew_timesheet_preparations', function (Blueprint $table) {
                $table->index(['payroll_period_id', 'status'], 'ctp_period_status_idx');
            });
        }
