...
export default function Clients({ clients }: { clients: ClientRow[] }) {
    ...
    const submit = () => {
        if (current) {
            form.put(`/settings/master-data/clients/${current.id}`, {
                preserveScroll: true,
        ...
        form.post('/settings/master-data/clients', {
            preserveScroll: true,
            onSuccess: () => setSheetOpen(false),
        });
    };

    const requestDelete = (row: ClientRow) => {
        setCurrent(row);
        setDeleteOpen(true);
    };
    ...
    const confirmDelete = () => {
        ...
        router.delete(`/settings/master-data/clients/${current.id}`, {
            preserveScroll: true,
            onFinish: () => {
                setDeleteOpen(false);
                setCurrent(null);
    ...
    const toggleActive = (row: ClientRow) => {
        router.put(
            `/settings/master-data/clients/${row.id}`,
            {
                name: row.name,
                is_active: !row.is_active,
            },
    ...
    const runImport = () => {
        ...
        router.post(
            '/settings/master-data/clients/import',
            { file: importFile },
            {
                preserveScroll: true,
                forceFormData: true,
    ...
    return (
        <>
            ...
            <div className="space-y-6">
                <Heading variant="small" title="Clients" description="Manage clients used across the system." />
                    ...
                    <DialogHeader className="space-y-0 border-b border-border px-6 py-5 text-left sm:text-left">
                        <div className="flex gap-4">
                            ...
                            <div className="min-w-0 space-y-1.5 pt-0.5">
                                <DialogTitle className="text-xl leading-tight">Import clients</DialogTitle>
                                <DialogDescription>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="rounded-xl border border-border/80 bg-muted/20 p-4">
                                ...
                                <Button variant="secondary" type="button" className="mt-3 w-full sm:w-auto" asChild>
                                    <a href="/settings/master-data/clients/import/template">
            ...
            <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
                <SheetContent side="right" className="w-full sm:max-w-md border-white/5 bg-black/60 backdrop-blur-3xl p-0 flex flex-col">
                    <SheetHeader className="p-8 pb-6 border-b border-white/5">
                        <SheetTitle className="text-xl font-bold tracking-tight text-white">{current ? 'Edit client' : 'New client'}</SheetTitle>
            ...
            <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
                <AlertDialogContent className="glass-card">
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            {current ? `This will permanently delete “${current.name}”.` : 'This will permanently delete this client.'}
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
