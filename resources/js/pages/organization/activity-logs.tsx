...
const HIDDEN_KEYS = new Set(['id', 'company_id', 'created_at', 'updated_at', 'deleted_at', 'remember_token', 'password']);
        ...
        <>
            ...
            <div className="mx-auto w-full max-w-6xl space-y-6">
                <div className="flex flex-col gap-1">
                    <div className="text-2xl font-semibold tracking-tight">Activity logs</div>
                    <div className="text-sm text-muted-foreground">Track changes across your organization data.</div>
                </div>
                        ...
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <div className="flex gap-2">
                                <Button
                                    ...
                                    variant="secondary"
                                    onClick={() => {
                                        const data = { q: '', event: '', subject: '', date_from: filters.date_from ?? '', date_to: filters.date_to ?? '' };
                                        form.setData(data);
                                        list.visit({ ...data, page: null });
                                    }}
                                >
                ...
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between">
                        ...
                        <div className="text-sm text-muted-foreground">{pagination.total} item(s)</div>
                    </CardHeader>
                    <CardContent className="p-0">
                        {logs.length === 0 ? (
                            ...
                            <div className="divide-y">
                                {logs.map((log) => {
                                    const changedKeys = pickChangedKeys(log.old_values, log.new_values).filter((k) => !HIDDEN_KEYS.has(k));
                                    const previewKeys = changedKeys.slice(0, 3);
                                    const isOpen = openId === log.id;
                                    const title = buildSummary(log);

                                    return (
                                        <Collapsible key={log.id} open={isOpen} onOpenChange={(v) => setOpenId(v ? log.id : null)}>
                                                            ...
                                                            {previewKeys.length > 0 && (
                                                                <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground/80">
                                                                    <span className="rounded-md bg-muted px-2 py-1">
                                                                        {previewKeys.join(', ')}
                                                                        {changedKeys.length > previewKeys.length ? ` +${changedKeys.length - previewKeys.length} more` : ''}
                                                                    </span>
                                                                </div>
                                                            )}
                                                        </div>
                                                        ...
                                                        <div className="flex shrink-0 flex-col items-end gap-1">
                                                            <div className="text-sm text-muted-foreground">
                                                                {log.causer ? (
                                                                    <span>
                                                                        {log.causer.name}{' '}
                                                                        <span className="text-muted-foreground/70">({log.causer.email})</span>
                                                                    </span>
                                                                ) : 'System'}
                                                            </div>
                                                    ...
                                                    <div className="grid gap-4 rounded-lg border bg-card p-4 md:grid-cols-2">
                                                        {changedKeys.length > 0 && (
                                                            <div className="md:col-span-2 space-y-2">
                                                                <div className="mb-2 text-sm font-medium">Changed fields</div>
