...
export function SeaServiceImportDialog({
    open,
    onOpenChange,
    employeeId,
}: SeaServiceImportDialogProps) {
                ...
                <DialogHeader className="space-y-0 border-b border-border px-6 py-5 text-left sm:text-left">
                    <div className="flex gap-4">
                        ...
                        <div className="min-w-0 space-y-1.5 pt-0.5">
                            ...
                            <DialogDescription>
                                Rows are appended to this employee&apos;s sea service history.
                                Vessel type and rank must match active master data names exactly.
                            </DialogDescription>
                        </div>
                    </div>
                </DialogHeader>
                ...
                <div className="space-y-5 px-6 py-5">
                    <Alert className="border-border/80 bg-muted/40">
                        ...
                        <AlertDescription>
                            ...
                            <ul className="list-inside list-disc space-y-1 text-muted-foreground">
                                <li>
                                    <span className="font-medium text-foreground">vessel_type</span>{' '}
                                    — required (aliases: Vessel type, Type)
                                </li>
                                <li>
                                    <span className="font-medium text-foreground">vessel_name</span>{' '}
                                    — required (aliases: Vessel name, Vessel)
                                </li>
                                <li>
                                    <span className="font-medium text-foreground">rank</span> — required
                                    (aliases: Rank name, Position)
                                </li>
                                <li>
                                    <span className="font-medium text-foreground">total_months</span> —
                                    required (aliases: Total months, Months)
                                </li>
                                <li>
                                    <span className="font-medium text-foreground">total_days</span> —
                                    required (aliases: Total days, Days)
                                </li>
                                <li>
                                    <span className="font-medium text-foreground">grt</span> — optional
                                </li>
                                <li>
                                    <span className="font-medium text-foreground">bhp</span> — optional
                                </li>
                                <li>
                                    <span className="font-medium text-foreground">client</span> — optional
                                    (must match an active client name)
                                </li>
                                <li>
                                    <span className="font-medium text-foreground">is_offshore</span> —
                    ...
                    <div className="space-y-2">
                        ...
                        <input
                            ...
                            className="sr-only"
                            id={`sea-service-import-${employeeId}`}
                            onChange={(event) => {
