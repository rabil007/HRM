...
export default function Documents({ documents, pagination, counts, active_status, search }: Props) {
    ...
    useEffect(() => {
        ...
        debounceRef.current = setTimeout(() => {
            navigate({ status: active_status, search: searchInput, page: null });
        }, 400);
        return () => {
            if (debounceRef.current) clearTimeout(debounceRef.current);
        };
                    ...
                    <div className="flex gap-1 bg-muted/40 rounded-lg p-1 border border-border">
                        {FILTER_TABS.map((t) => (
                            <button
                                key={String(t.key)}
                                type="button"
                                onClick={() => navigate({ status: t.key, search, page: null })}
                            ...
                            {documents.length === 0 ? (
                                ...
                                documents.map((doc) => (
                                    <TableRow key={doc.id} className="border-border/40 hover:bg-accent/40">
                                        ...
                                        <TableCell className="text-muted-foreground/80 text-xs">{doc.document_type ?? '—'}</TableCell>
                                        ...
                                        <div className="flex items-center justify-between px-2 py-1">
                                            ...
                                            <div className="flex items-center gap-1">
                                                <button
                                                    ...
                                                    onClick={() => navigate({ status: active_status, search, page: pagination.current_page - 1 })}
                                                    ...
                                                    .map((p, i) =>
                                                        p === '...' ? (
                                                            ...
                                                            <button
                                                                key={p}
                                                                type="button"
                                                                onClick={() => navigate({ status: active_status, search, page: p as number })}
