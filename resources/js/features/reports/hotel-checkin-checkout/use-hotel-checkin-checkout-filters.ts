...
import { index } from '@/routes/organization/reports/hotel-checkin-checkout';
import type { HotelCheckInCheckoutFilters } from './types';

function clean(
    filters: Partial<HotelCheckInCheckoutFilters> & {
        page?: number;
        per_page?: number;
    },
...
export function useHotelCheckInCheckoutFilters(
    ...
    const visit = useCallback(
        (
            next: Partial<HotelCheckInCheckoutFilters> & {
                page?: number;
                per_page?: number;
            },
        ) => {
            ...
            router.get(
                ...
                clean({ ...filters, per_page: perPage, ...next }),
                {
                    ...
                    preserveScroll: true,
                    replace: true,
                    only: ['stays', 'pagination', 'summary', 'filters'],
                    onFinish: () => {
                        setIsLoading(false);
                    },
                },
