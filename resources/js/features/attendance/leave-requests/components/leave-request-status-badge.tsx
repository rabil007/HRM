...
const STATUS_STYLES: Record<LeaveRequestStatus, string> = {
    ...
    cancelled: 'bg-muted/60 text-muted-foreground border-border dark:bg-zinc-500/10 dark:text-zinc-200 dark:border-zinc-500/20',
};
...
export function LeaveRequestStatusBadge({ status }: { status: LeaveRequestStatus }) {
    return (
        <Badge className={cn('text-[10px] uppercase font-bold tracking-wider border', STATUS_STYLES[status])}>
