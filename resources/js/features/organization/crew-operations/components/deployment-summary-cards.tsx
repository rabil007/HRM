...
export function CrewOperationsDeploymentSummaryCards({
    ...
    const cardClass = 'min-w-0 flex-1 text-left sm:min-w-[calc(33.333%-0.5rem)] md:min-w-[calc(20%-0.6rem)] lg:min-w-0';

    return (
        <div className="flex flex-wrap gap-3">
            <Link key={TOTAL_ITEM.key} href={deploymentFilterHref(TOTAL_ITEM.key)} className={cardClass}>
                <Card className={cn('h-full w-full transition-colors', TOTAL_ITEM.cardClass)}>
                    <CardContent className="p-4">
                        <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60">
                            {TOTAL_ITEM.label}
                        </p>
                        <p className={cn('mt-1 text-2xl font-bold tabular-nums', TOTAL_ITEM.valueClass)}>
                            {summary.total ?? 0}
                        </p>
                    </CardContent>
                </Card>
            ...
            {SUMMARY_ITEMS.map((item) => (
                <Link key={item.key} href={deploymentFilterHref(item.key)} className={cardClass}>
                    <Card className={cn('h-full w-full transition-colors', item.cardClass)}>
                        <CardContent className="p-4">
                            <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60">
                                {item.label}
                            </p>
