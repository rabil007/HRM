...
test('build department employee tree can restrict visible department nodes', function () {
    $country = Country::query()->create([
        'code' => 'BDR',
        'name' => 'Restrictland',
        'dial_code' => '+974',
    ...
    $tree = BuildDepartmentEmployeeTree::for(
        ...
        allowedDepartmentIds: [$marineDepartment->id],
    );

    $departmentIds = collect($tree)
        ->flatMap(fn (array $node): array => $node['id'] === null ? [] : [(int) $node['id']])
        ->all();

    expect($departmentIds)->toBe([$marineDepartment->id]);
});
