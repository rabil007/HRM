...
test('users can bulk delete documents across employees from the documents index', function () {
    ...
    expect(EmployeeDocument::query()->whereKey($docA->id)->exists())->toBeFalse();
    expect(EmployeeDocument::query()->whereKey($docB->id)->exists())->toBeFalse();
    Storage::disk('public')->assertMissing($pathA);
...
test('company bulk delete rejects documents from another company', function () {
    ...
    $this->delete(route('organization.documents.files.bulk-destroy'), [
        'document_ids' => [$document->id],
    ])->assertNotFound();
...
test('users without delete permission cannot company bulk delete documents', function () {
    $user = User::factory()->create();
    $this->actingAs($user);
