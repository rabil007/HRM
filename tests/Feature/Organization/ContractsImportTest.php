...
test('office contracts template excludes crew-only employees', function () {
    ...
    for ($row = ContractsImport::DATA_START_ROW; $row <= $sheet->getHighestDataRow(); $row++) {
        $employeeNumbers[] = (string) $sheet->getCell('B'.$row)->getValue();
    }
...
test('office contracts template lists active employees and prefills matching office contracts', function () {
    ['user' => $user, 'company' => $company, 'officeEmployee' => $officeEmployee, 'blankEmployee' => $blankEmployee, 'officeContract' => $officeContract] = makeContractsImportFixtures();
    ...
    for ($row = ContractsImport::DATA_START_ROW; $row <= $sheet->getHighestDataRow(); $row++) {
        $rowsByEmployeeNo[(string) $sheet->getCell('B'.$row)->getValue()] = [
            'contract_id' => $sheet->getCell('A'.$row)->getValue(),
            'contract_type' => $sheet->getCell('D'.$row)->getValue(),
        ];
    }
    ...
    expect($rowsByEmployeeNo[$blankEmployee->employee_no]['contract_type'])->toBeNull()
        ...
        ->and((string) $rowsByEmployeeNo[$officeEmployee->employee_no]['contract_id'])
        ->toBe((string) $officeContract->id);
...
test('crew contracts template includes crew allowance columns', function () {
    ...
    expect($sheet)->not->toBeNull()
        ->and($sheet->getCell('J1')->getValue())->toBe('Supplementary Allowance')
        ->and($sheet->getCell('K1')->getValue())->toBe('Site Allowance');
...
test('contracts import updates existing contracts by contract id', function () {
...
function makeContractsImportFile(PayrollCategory $payrollCategory, array $rows): UploadedFile
{
    ...
    foreach ($rows as $row) {
        $sheet->setCellValueByColumnAndRow(1, $rowNumber, $row['contract_id'] ?? null);
        $sheet->setCellValueByColumnAndRow(2, $rowNumber, $row['employee_no'] ?? null);
        $sheet->setCellValueByColumnAndRow(3, $rowNumber, $row['name'] ?? null);
        $sheet->setCellValueByColumnAndRow(4, $rowNumber, $row['contract_type'] ?? null);
        $sheet->setCellValueByColumnAndRow(5, $rowNumber, $row['start_date'] ?? null);
        $sheet->setCellValueByColumnAndRow(6, $rowNumber, $row['end_date'] ?? null);
        $sheet->setCellValueByColumnAndRow(7, $rowNumber, $row['labor_contract_id'] ?? null);
        $sheet->setCellValueByColumnAndRow(8, $rowNumber, $row['status'] ?? null);
        $sheet->setCellValueByColumnAndRow(9, $rowNumber, $row['basic_salary'] ?? null);

        if ($payrollCategory === PayrollCategory::Office) {
            $sheet->setCellValueByColumnAndRow(10, $rowNumber, $row['housing_allowance'] ?? null);
            $sheet->setCellValueByColumnAndRow(11, $rowNumber, $row['transport_allowance'] ?? null);
            $sheet->setCellValueByColumnAndRow(12, $rowNumber, $row['other_allowances'] ?? null);
            $sheet->setCellValueByColumnAndRow(13, $rowNumber, $row['note'] ?? null);
        } else {
            $sheet->setCellValueByColumnAndRow(10, $rowNumber, $row['supplementary_allowance'] ?? null);
            $sheet->setCellValueByColumnAndRow(11, $rowNumber, $row['site_allowance'] ?? null);
