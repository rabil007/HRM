...
test('segment save updates segments with pay category and dates', function () {
    $fixtures = makeMultiSegmentManualTimesheetFixtures();
    ...
    expect($segment->from_date?->toDateString())->toBe('2026-07-01')
        ->and($segment->to_date?->toDateString())->toBe('2026-07-12')
        ->and($segment->remarks)->toBe('dates only');
});

test('payroll show exposes segment values for the movement editor', function () {
    $fixtures = makeMultiSegmentManualTimesheetFixtures();
    ...
    $this->actingAs($fixtures['user'])
        ...
        ->assertInertia(fn ($page) => $page
            ...
            ->has('rows')
            ->where('rows.0.timesheet.segments.0.pay_category', CrewTimesheetPayCategory::Onsite->value)
