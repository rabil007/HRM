...
final class EmployeeProfilePageData
{
    ...
    public static function for(Employee $employee, int $companyId, Request $request): array
    {
        ...
        return [
            ...
            'can' => [
                ...
                'education_delete' => $authUser?->can('education.delete') ?? false,
                'work_experience_view' => $authUser?->can('work_experience.view') ?? false,
                'work_experience_create' => $authUser?->can('work_experience.create') ?? false,
                'work_experience_update' => $authUser?->can('work_experience.update') ?? false,
                'work_experience_delete' => $authUser?->can('work_experience.delete') ?? false,
                'work_experience_import' => $authUser?->can('work_experience.import') ?? false,
                'vaccination_view' => $authUser?->can('vaccination.view') ?? false,
                'vaccination_create' => $authUser?->can('vaccination.create') ?? false,
                'vaccination_update' => $authUser?->can('vaccination.update') ?? false,
                'vaccination_delete' => $authUser?->can('vaccination.delete') ?? false,
                'vaccination_import' => $authUser?->can('vaccination.import') ?? false,
                'languages_view' => $authUser?->can('languages.view') ?? false,
                'languages_create' => $authUser?->can('languages.create') ?? false,
                'languages_update' => $authUser?->can('languages.update') ?? false,
                'languages_delete' => $authUser?->can('languages.delete') ?? false,
                'salary_certificate_print' => $authUser?->can('employees.salary_certificate.print') ?? false,
                'salary_declaration_print' => $authUser?->can('employees.salary_declaration.print') ?? false,
                'bank_accounts_view' => $authUser?->can('bank_accounts.view') ?? false,
                'bank_accounts_create' => $authUser?->can('bank_accounts.create') ?? false,
                'bank_accounts_update' => $authUser?->can('bank_accounts.update') ?? false,
                'bank_accounts_delete' => $authUser?->can('bank_accounts.delete') ?? false,
                'bank_accounts_manage' => ($authUser?->can('bank_accounts.create') || $authUser?->can('bank_accounts.update')) ?? false,
                'sea_service_view' => $authUser?->can('sea_services.view') ?? false,
                'sea_service_create' => $authUser?->can('sea_services.create') ?? false,
                'sea_service_update' => $authUser?->can('sea_services.update') ?? false,
                'sea_service_delete' => $authUser?->can('sea_services.delete') ?? false,
                'sea_service_import' => $authUser?->can('sea_services.import') ?? false,
                'sea_service_manage' => ($authUser?->can('sea_services.create') || $authUser?->can('sea_services.update')) ?? false,
                'training_view' => $authUser?->can('training.view'),
                'training_create' => $authUser?->can('training.create'),
    ...
    private static function permissionsPayload(?User $authUser): array
    {
        return [
            ...
            'sea_service_import' => $authUser?->can('sea_services.import') ?? false,
            'sea_service_manage' => ($authUser?->can('sea_services.create') || $authUser?->can('sea_services.update')) ?? false,
            'training_view' => $authUser?->can('training.view') ?? false,
            'training_create' => $authUser?->can('training.create') ?? false,
