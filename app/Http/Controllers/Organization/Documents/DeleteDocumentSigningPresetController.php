...
class DeleteDocumentSigningPresetController extends Controller
{
    public function __invoke(
        Request $request,
        DocumentSigningPreset $signingPreset,
        DeleteDocumentSigningPreset $action,
    ): RedirectResponse {
        abort_unless($request->user()?->can('documents.signing-presets.delete') ?? false, 403);
        $companyId = (int) $request->attributes->get('current_company_id');
        abort_unless((int) $signingPreset->company_id === $companyId, 404);

        $action->handle($signingPreset, $request->user(), $companyId);

        return back()->with('success', 'Signing preset deleted.');
    }
}
